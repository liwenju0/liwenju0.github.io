...
再根据这个`mask`的信息，创造一个包含要写入的汉字的二值图，这个`mask`也变成一个叫`hint`的二值数组。
...
`text_ids`的`shape`是[512, 3] 这是t5返回的位置编码，是全0。之所以是3，是为了和图像的位置编码对齐。
...
这里单独强调一下。`mask_image`就是`mask`， m`asked_image`是原始图像扣去`mask`后的图像，所以是`masked`。
